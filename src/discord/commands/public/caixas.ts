...
async function getRandomCards(rarity: string, amount: number, userCards: CardInterface[]) {
    ...
    const newCards = availableCards.filter(card => 
        !userCards.includes(card.id.toString())
    );
    ...
    async run(interaction) {
        ...
        if (subcommand === "info") {
            const embed = createEmbed({
                ...
                fields: Object.entries(boxes).map(([key, data]) => ({
        ...
        if (subcommand === "comprar") {
            ...
            if (user.ryos < box.price) {
                return interaction.editReply(res.danger(
            ...
            const cards = await getRandomCards(box.cardRarity, cardAmount, user.cards);

            if (cards.length === 0) {
                return interaction.editReply(res.danger(
                    `${icon.danger} Não há mais cards ${box.cardRarity.toLowerCase()} disponíveis para você colecionar!`
                ));
            }
            ...
            const cardEmbeds = cards.map(card => createEmbed({
                color: box.color as ColorResolvable,
                title: `${card.name}`,
