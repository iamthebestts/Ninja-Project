...
import { boxes, BoxType, settings } from "#settings";
...
createResponder({
    ...
    async run(interaction, params) {
        ...
        const numCards = Math.floor(Math.random() * (box.maxCards - box.minCards + 1)) + box.minCards;
        const selectedCards = [];
        ...
        for (const card of selectedCards) {
            const hasCard = user.cards.some(c => c.toString() === card._id.toString());
            if (hasCard) {
        ...
        if (refundTotal > 0) await user.addRyos(refundTotal);
        ...
        const embed = createEmbed({
            ...
            description: brBuilder(
                selectedCards.map(card => {
                    const isRepetido = user.cards.some(c => c.toString() === card._id.toString());
